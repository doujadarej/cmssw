...
class TrackingPerf : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
 ...
 std::vector<float> tree_jet_idxTrack;
 ...
 std::vector<float> tree_Calojet_vx;
 std::vector<float> tree_Calojet_vy;
 std::vector<float> tree_Calojet_vz;
 ...
 std::vector<float> tree_jet08_vx;
 std::vector<float> tree_jet08_vy;
 std::vector<float> tree_jet08_vz;
 ...
 std::vector<float> tree_Calojet08_vx;
 std::vector<float> tree_Calojet08_vy;
 std::vector<float> tree_Calojet08_vz;
 ...
 //--------------------------------
  ...
  std::vector<float> tree_ak8GenJet_vx;
  std::vector<float> tree_ak8GenJet_vy;
  std::vector<float> tree_ak8GenJet_vz;
  std::vector<float> tree_ak8GenJet_mass; 
  std::vector<float> tree_ak8GenJet_energy; 
  std::vector<float> tree_ak8GenJet_status; 
  std::vector<float> tree_ak8GenJet_pdgId; 
 ...
 //--------------------------------
  ...
  std::vector<float> tree_muon_charge; 
	...
	runOnData_ (        							 iConfig.getUntrackedParameter<bool>("runOnData"))
{
  ...
  //
   ...
   smalltree->Branch("tree_Calojet_vx"  , &tree_Calojet_vx);
   smalltree->Branch("tree_Calojet_vy" , &tree_Calojet_vy);
   smalltree->Branch("tree_Calojet_vz" , &tree_Calojet_vz);
   ...
   smalltree->Branch("tree_jet08_vx"  , &tree_jet08_vx);
   smalltree->Branch("tree_jet08_vy" , &tree_jet08_vy);
   smalltree->Branch("tree_jet08_vz" , &tree_jet08_vz);
   ...
   smalltree->Branch("tree_Calojet08_vx"  , &tree_Calojet08_vx);
   smalltree->Branch("tree_Calojet08_vy"  , &tree_Calojet08_vy);
   smalltree->Branch("tree_Calojet08_vz"  , &tree_Calojet08_vz);
   ...
   smalltree->Branch("tree_ak8GenJet_vx"  ,   &tree_ak8GenJet_vx);
   smalltree->Branch("tree_ak8GenJet_vy" ,    &tree_ak8GenJet_vy);
   smalltree->Branch("tree_ak8GenJet_vz" ,    &tree_ak8GenJet_vz);
   smalltree->Branch("tree_ak8GenJet_mass",   &tree_ak8GenJet_mass); 
   smalltree->Branch("tree_ak8GenJet_energy", &tree_ak8GenJet_energy); 
   smalltree->Branch("tree_ak8GenJet_status", &tree_ak8GenJet_status); 
   smalltree->Branch("tree_ak8GenJet_pdgId",  &tree_ak8GenJet_pdgId); 
   ...
   smalltree->Branch("tree_muon_CutBasedIdLoose",        &tree_muon_CutBasedIdLoose        ); 
   ...
   smalltree->Branch("tree_muon_CutBasedIdTight",        &tree_muon_CutBasedIdTight        );
...
///////// vertex refitting used to be here 
   ...
   if(!runOnData_){
     ...
     for (auto const & genParticle : *genParticles)
     {
      ...
      for (auto const & genJet : *ak8GenJets)
      {
        ...
        tree_ak8GenJet_status.push_back(genJet.status());
        tree_ak8GenJet_pdgId.push_back(genJet.pdgId());
      }
    }
    /// Electrons 
    ...
    for (auto const & muon : *muons)
    {
      ...
      tree_muon_dxy.push_back(      muon.bestTrack()->dxy(bs));
      tree_muon_dxyError.push_back( muon.bestTrack()->dxyError());
      tree_muon_dz.push_back(       muon.bestTrack()->dz(bs.position()));
    ...
    for (const auto& muon: *muons){
  /*if (muon.passed(reco::Muon::PFIsoVeryTight))  (tree_muon_PFisoVeryTight.push_back(true)); else (tree_muon_PFisoVeryTight.push_back(false));
    if (muon.passed(reco::Muon::PFIsoTight))      (tree_muon_PFisoTight.push_back(true));else (tree_muon_PFisoTight.push_back(false)); 
    if (muon.passed(reco::Muon::PFIsoMedium))     (tree_muon_PFisoMedium.push_back(true));else (tree_muon_PFisoMedium.push_back(false));
    if (muon.passed(reco::Muon::PFIsoLoose ))     (tree_muon_PFisoLoose.push_back(true));else (tree_muon_PFisoLoose.push_back(false));
    if (muon.passed(reco::Muon::MvaLoose ))       (tree_muon_MVAisoLoose.push_back(true));else (tree_muon_MVAisoLoose.push_back(true));
    if (muon.passed(reco::Muon::MvaMedium ))      (tree_muon_MVAisoMedium.push_back(true));else (tree_muon_MVAisoMedium.push_back(false));
    if (muon.passed(reco::Muon::MvaTight ))       (tree_muon_MVAisoTight.push_back(true));else (tree_muon_MVAisoTight.push_back(false));
    if (muon.isGlobalMuon())                      (tree_muon_isGlobalMuon.push_back(true));else (tree_muon_isGlobalMuon.push_back(false));
    if (muon.isStandAloneMuon())                  (tree_muon_isStandAloneMuon.push_back(true));else (tree_muon_isStandAloneMuon.push_back(false));*/
      ...
      tree_muon_isGlobalMuon.    push_back(   muon.passed(muon.isGlobalMuon())    );
      tree_muon_isStandAloneMuon.push_back(   muon.passed(muon.isStandAloneMuon()));
      tree_muon_CutBasedIdLoose	            .push_back(   muon.passed(reco::Muon::CutBasedIdLoose));
      ...
      tree_muon_CutBasedIdTight	            .push_back(   muon.passed(reco::Muon::CutBasedIdTight));
  ...
  ////// chosing the best tracks for muons 
   ...
   for (const auto& muon: *muons){
   	if(muon.passed(reco::Muon::PFIsoLoose) && muon.passed(reco::Muon::CutBasedIdMedium ) )  muonTracks.push_back(muon.bestTrack()); 
   }
  ...
  else
  {
   ...
   for(int ij=0;ij<nJet;ij++){
     ...
     for(unsigned int iTrack=0; iTrack< tree_track_pt.size() ; iTrack++) {
       ...
       if(thejet.DeltaR(track) < 0.5 ) tree_jet_idxTrack.push_back(iTrack);    
     }
   }

   for(int ij=0;ij< int(caloJets->size());ij++){
     ...
     tree_Calojet_vx.push_back(jet.vx());
     tree_Calojet_vy.push_back(jet.vy());
     tree_Calojet_vz.push_back(jet.vz());
   }
   ...
   for(int ij=0;ij< int(ak8jets->size() );ij++){
     ...
     tree_jet08_vx.push_back(jet.vx());
     tree_jet08_vy.push_back(jet.vy());
...
void TrackingPerf::clearVariables() {
 ...
 //-----------------------
  //fill the tree per track
   ...
   tree_Calojet_vx.clear();
   tree_Calojet_vy.clear();
   tree_Calojet_vz.clear();
   ...
   tree_Calojet08_vx.clear();
   tree_Calojet08_vy.clear();
   tree_Calojet08_vz.clear();
   ...
   tree_ak8GenJet_vx.clear();
   tree_ak8GenJet_vy.clear();
   tree_ak8GenJet_vz.clear();
   tree_ak8GenJet_mass.clear(); 
   tree_ak8GenJet_energy.clear(); 
   tree_ak8GenJet_status.clear(); 
   tree_ak8GenJet_pdgId.clear(); 
