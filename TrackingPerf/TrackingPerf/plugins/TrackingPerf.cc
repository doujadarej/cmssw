...
class TrackingPerf : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
  ...
  const edm::EDGetTokenT<reco::VertexCompositeCandidateCollection> KshortToken_;
  const edm::EDGetTokenT<reco::VertexCompositeCandidateCollection> LambdaToken_;
 ...
 // std::vector<int>              tree_track_simtrack_matchedHit	;
  ...
  // V0 Candidates info
  //--------------------------------
  std::vector<float> tree_Kshort_mass;  
  std::vector<double> tree_Kshort_vx;
  std::vector<double> tree_Kshort_vy; 
  std::vector<double> tree_Kshort_vz; 
  std::vector< double > tree_Kshort_pt;
  std::vector< double > tree_Kshort_eta;
  std::vector< double > tree_Kshort_phi;


  std::vector<float> tree_Lambda_mass;
  std::vector<double> tree_Lambda_vx;
  std::vector<double> tree_Lambda_vy; 
  std::vector<double> tree_Lambda_vz;
  std::vector< double > tree_Lambda_pt;
  std::vector< double > tree_Lambda_eta;
  std::vector< double > tree_Lambda_phi;
...
TrackingPerf::TrackingPerf(const edm::ParameterSet& iConfig):
  ...
  vertexToken_(consumes<reco::VertexCollection>(iConfig.getUntrackedParameter<edm::InputTag>("vertices"))),
    KshortToken_(consumes<reco::VertexCompositeCandidateCollection> (iConfig.getParameter<edm::InputTag>("theKshorts"))),
  LambdaToken_(consumes<reco::VertexCompositeCandidateCollection> (iConfig.getParameter<edm::InputTag>("theLambdas"))),
	...
	useCluster_(iConfig.getUntrackedParameter<bool>("useCluster"))
{
 ...
 //  
  ...
  //
   ...
   /////////
      smalltree->Branch("tree_Kshort_mass", &tree_Kshort_mass);
   smalltree->Branch("tree_Kshort_vx", &tree_Kshort_vx);
   smalltree->Branch("tree_Kshort_vy", &tree_Kshort_vy);
   smalltree->Branch("tree_Kshort_vz", &tree_Kshort_vz);
   smalltree->Branch("tree_Kshort_pt",		     &tree_Kshort_pt);		      
   smalltree->Branch("tree_Kshort_eta",  	     &tree_Kshort_eta ); 	      
   smalltree->Branch("tree_Kshort_phi",  	     &tree_Kshort_phi ); 


   smalltree->Branch("tree_Lambda_mass", &tree_Lambda_mass);
   smalltree->Branch("tree_Lambda_vx", &tree_Lambda_vx);
   smalltree->Branch("tree_Lambda_vy", &tree_Lambda_vy);
   smalltree->Branch("tree_Lambda_vz", &tree_Lambda_vz);
   smalltree->Branch("tree_Lambda_pt",		     &tree_Lambda_pt);		      
   smalltree->Branch("tree_Lambda_eta",  	     &tree_Lambda_eta ); 	      
   smalltree->Branch("tree_Lambda_phi",  	     &tree_Lambda_phi ); 
...
TrackingPerf::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   iEvent.getByToken( KshortToken_, theKshorts); 

   edm::Handle<reco::VertexCompositeCandidateCollection> theLambdas;
   iEvent.getByToken( LambdaToken_, theLambdas);
   ...
   for(edm::View<Track>::size_type i=0; i<tracks.size(); ++i) {
     ...
     if(tracks[i].pt() < 1 || fabs(tracks[i].eta()) > 3) continue;
   ...
   for (auto const & Kshort : *theKshorts) {
    
    tree_Kshort_mass.push_back(Kshort.mass());
    tree_Kshort_vx.push_back(Kshort.vx());
    tree_Kshort_vy.push_back(Kshort.vy());
    tree_Kshort_vz.push_back(Kshort.vz());
    tree_Kshort_pt.push_back(Kshort.pt());
    tree_Kshort_eta.push_back(Kshort.eta());
    tree_Kshort_phi.push_back(Kshort.phi());

   }

   for (auto const & Lambda : *theLambdas) {
    
    tree_Lambda_mass.push_back(Lambda.mass());
    tree_Lambda_vx.push_back(Lambda.vx());
    tree_Lambda_vy.push_back(Lambda.vy());
    tree_Lambda_vz.push_back(Lambda.vz());
    tree_Lambda_pt.push_back(Lambda.pt());
    tree_Lambda_eta.push_back(Lambda.eta());
    tree_Lambda_phi.push_back(Lambda.phi());
...
void TrackingPerf::clearVariables() {
 //-----------------------
  //fill the tree per track
   ...
   tree_Kshort_mass.clear();
   tree_Kshort_vx.clear(); 
   tree_Kshort_vy.clear();
   tree_Kshort_vz.clear();
   tree_Kshort_pt.clear(); 
   tree_Kshort_eta.clear();
   tree_Kshort_phi.clear();

   tree_Lambda_mass.clear(); 
   tree_Lambda_vx.clear(); 
   tree_Lambda_vy.clear();
   tree_Lambda_vz.clear(); 
   tree_Lambda_pt.clear(); 
   tree_Lambda_eta.clear();
   tree_Lambda_phi.clear();
